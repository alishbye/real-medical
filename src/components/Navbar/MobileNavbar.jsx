...
export default function MobileNavbar() {
  ...
  return (
    <div className="md:hidden w-full bg-white p-4 shadow-sm relative z-50">
      ...
      <img 
        src="logo.png" 
        alt="Company Logo" 
       className="h-10 w-10 object-contain transform scale-425"
      />
    </div>

        <div className="flex items-center space-x-4">
          <DollarSign className="w-7 h-7 cursor-pointer" />
          <Globe className="w-7 h-7 cursor-pointer" />
          <User className="w-7 h-7 cursor-pointer" />
          <MessageCircle className="w-7 h-7 cursor-pointer" />
          <div className="relative cursor-pointer">
            <ShoppingBag className="w-7 h-7" />
            <span className="absolute -top-2 -right-2 text-xs bg-black text-white rounded-full w-4 h-4 flex items-center justify-center">0</span>
          </div>
        </div>
      </div>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 z-40 transition-opacity duration-300 ${menuOpen ? 'bg-black/30' : 'pointer-events-none opacity-0'}`}
        onClick={() => setMenuOpen(false)}
      >
        {/* Sliding Menu */}
        <div
          className={`fixed top-0 left-0 h-full w-64 bg-white shadow-lg transition-transform duration-300 ease-in-out z-50 ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}
